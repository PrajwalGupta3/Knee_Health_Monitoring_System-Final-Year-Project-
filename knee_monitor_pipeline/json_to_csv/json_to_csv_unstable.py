...
with open("knee_monitor_pipeline/data/raw/unstable.json") as f:
    data = json.load(f)
...
if not mpu_df.empty:
    mpu_df.to_csv("knee_monitor_pipeline/data/processed/mpu_readings_unstable.csv", index=False)
if not bmp_df.empty:
    bmp_df.to_csv("knee_monitor_pipeline/data/processed/bmp_readings_unstable.csv", index=False)