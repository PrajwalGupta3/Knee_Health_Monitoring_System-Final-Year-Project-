...
with open("knee_monitor_pipeline/data/raw/recovery.json") as f:
    data = json.load(f)
...
if not mpu_df.empty:
    mpu_df.to_csv("knee_monitor_pipeline/data/processed/mpu_readings_recovery.csv", index=False)
if not bmp_df.empty:
    bmp_df.to_csv("knee_monitor_pipeline/data/processed/bmp_readings_recovery.csv", index=False)