...
ROLLING_WINDOW = 5      #At 10Hz, this is a 0.5-second smooth, which is reasonable for gait data
...
plots_dir = Path("knee_monitor_pipeline/data/plots/healthy_qc")
plots_dir.mkdir(parents=True, exist_ok=True)

try:
    #This script reads the NEW CSV created with the 10Hz data
    mpu_df = pd.read_csv(data_dir / "mpu_readings_healthy.csv")
    bmp_df = pd.read_csv(data_dir / "bmp_readings_healthy.csv")
...
for i, (mpu_time, bmp_time, time_offset) in enumerate(paired_sessions, start=1):
    ...
    print(f"Processing {session_id}: {len(mpu_block)} readings @ {fs_mpu:.2f} Hz")
...
if METADATA_LOADED and not baseline_df.empty:
    try:
        ...
        if 'subject_id' in baseline_df.columns:
            ...
            print("[WARN] 'subject_id' column not found after merging with session_meta_df. Skipping user_meta_df merge.")
...
baseline_path = data_dir / "final_csv" / "healthy_baseline_qc.csv"
